...
# Set EMQX dashboard admin password
...
cat <<EOF > /opt/emqx/etc/emqx_authn_http.yaml
authn:
  - name: http_auth
...
# Let EMQX know where to find the auth config
export EMQX_AUTHN__CONFIG=/opt/emqx/etc/emqx_authn_http.yaml
