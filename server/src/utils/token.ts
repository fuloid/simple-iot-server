...
class Token {
    ...
    static async create(device_id: string): Promise<{ c:string; token?: string }> {
        try {
            ...
            logger.debug('[Token] Creating token with payload:', payload);
            ...
            logger.info('[Token] Token creation successful:', token);
            return { c: 'OK', token };
        } catch (err) {
            logger.error('[Token] Token creation or DB save failed:', err);
            return { c: 'ERROR' };
        }
    }
    ...
    static verify(token: string) {
        try {
            ...
            logger.debug('[Token] Token verified successfully:', decoded);
            return { c:"OK", device_id: (decoded as any).device_id };
        } catch (err) {
            logger.debug('[Token] Token verification failed:', err);
            return { c:"ERROR" };
        }
    }
    ...
    static async getDeviceToken(device_id: string): Promise<{ c:string; t?: string }> {
        try {
            ...
            if (!result) {
                logger.debug('[Token] Device not found in DB');
                return { c: 'ERROR' };
            }

            const { token, token_expires_at } = result;
            if (!token || !token_expires_at || new Date(token_expires_at).getTime() < Date.now()) {
                logger.debug('[Token] Token expired or missing, creating new one');
                return await this.create(device_id);
            }

            logger.debug('[Token] Valid token found in DB');
            return { c: 'OK', t: token };
        } catch (err) {
            logger.error('[Token] Error fetching device token:', err);
            return { c: 'ERROR' };
        }
    }
}
